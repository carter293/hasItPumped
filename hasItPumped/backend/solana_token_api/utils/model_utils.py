...
def load_model(model_path: str = "./solana_token_api/assets/model.ubj") -> xgb.XGBClassifier:
    """
...
def make_prediction(model: xgb.XGBClassifier, df: pd.DataFrame) -> Tuple[bool, float]:
    ...
    try:
        ...
        logger.info(f"Prediction: {'pre-peak' if is_pre_peak else 'post-peak'} with {confidence:.2f} confidence")
